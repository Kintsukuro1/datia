...
export const Header: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isHealthPopoverOpen || isMobileMenuOpen) {
      document.addEventListener('mousedown', handleClickOutside);
    }
    return () => {
      document.removeEventListener('mousedown', handleClickOutside);
    };
  }, [isHealthPopoverOpen, isMobileMenuOpen]);
      ...
      <div className="flex items-center space-x-3 sm:space-x-4">
        ...
        >
          ...
          <div className="truncate">
            ...
            <p className="text-[10px] sm:text-xs text-gray-400 hidden xs:block truncate">Democratización de Datos</p>
          </div>
        </button>
        ...
        <div className="md:hidden relative" ref={mobileMenuRef}>
          ...
          {isMobileMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-dark-surface border border-dark-border shadow-2xl p-3 z-50 space-y-2 animate-fadeIn">
              ...
              <div className={`p-2.5 rounded-xl border text-xs flex items-center space-x-2 ${badgeUI.bg}`}>
                {badgeUI.icon}
                <span className="font-medium text-[11px]">{badgeUI.label}</span>
              </div>
